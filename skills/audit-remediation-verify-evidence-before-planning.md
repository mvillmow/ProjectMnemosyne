...
>
> **v1.5.0 note (UNVERIFIED — plan only; ghost-scaffolding and ignored-file-semantics case):** Planning ProjectHephaestus issue #1468 (stale audit issue citing source/test directories for removed tooling). **No implementation workflow executed end-to-end, no tests run against the proposed fix, no CI** — this addition is `unverified`. The durable learning: do not restore deleted tooling just to satisfy stale path citations. First re-check the cited source and test paths with `find`, `git ls-files` for named paths plus `*.pyc`/`__pycache__`, `.gitignore` rules for ignored bytecode, and current governance docs that may explicitly say the old tooling must not be restored. The highest review risk is ignored-file semantics: an existing mirror-set helper or source-side phantom guard can intentionally filter ignored/no-source directories, which means `tests/unit/<name>/` containing only `__pycache__` can be invisible unless a dedicated direct test-directory phantom detector walks immediate test dirs. Treat any plan reliance on line numbers, policy docs, hook registration, console scripts, or helper behavior without same-turn verification as an explicit assumption for reviewer scrutiny.
>
...
- An audit cites source/test paths for tooling that may have been intentionally removed. Treat the paths as possible ghost scaffolding only after re-checking the live tree, tracked files, ignored bytecode artifacts, and current repo policy/docs.
- An existing source-side phantom guard or mirror-set helper filters ignored/no-source directories. Reviewers should focus on whether bytecode-only `tests/unit/<name>/` skeletons can slip through that filter.
- A plan depends on line numbers, docs, hook registrations, console scripts, or helper behavior that was not directly verified in the same turn. Mark each dependency as an assumption, not a fact.
...
17. **Re-check the cited source and test paths before choosing the fix direction.** Run `find` on every audit-cited source/test directory and `git ls-files` for the named paths, `*.pyc`, and `__pycache__`. A missing live path plus no tracked artifact is evidence for stale/ghost scaffolding; a tracked artifact is a cleanup bug; an ignored artifact is a guard-coverage bug.
18. **Validate current repo policy before restoring anything.** If governance docs say the old tooling/package was removed and must not be restored, do not recreate it just to satisfy stale citations. The right fix is usually a regression guard that keeps the ghost scaffolding from returning.
19. **Audit ignored-file semantics explicitly.** Helpers that compute source/test mirror sets often skip ignored paths, no-source dirs, or `__pycache__`. That is usually correct for mirror matching, but it can hide `tests/unit/<name>/` directories that contain only bytecode. Review the helper's filters and write down what they intentionally ignore.
20. **Keep the existing source-side phantom guard, then add the missing direct test-dir guard.** Do not weaken a working source-side check. Add a focused detector that walks immediate `tests/unit/<name>/` directories and reports ones that contain only ignored noise such as `__pycache__`/`.pyc`, then wire its reporter into the existing orchestrating check.
21. **Make the regression non-vacuous.** Include a synthetic `tests/unit/<name>/__pycache__/...pyc` fixture regression, a normal test-dir/non-phantom case, and a real-repo absence assertion so the guard proves both detection and current cleanliness.
22. **Name reviewer assumptions.** If the plan relies on exact line numbers, docs, hook registrations, console scripts, or helper behavior that were not opened and verified in the same turn, list them as assumptions. The reviewer should check those first.
...
# --- ghost-scaffolding / ignored-file-semantics audit (e.g. removed tooling dirs) [UNVERIFIED plan] ---
# 18. Re-check every audit-cited source/test path against the live tree
find <src-path> <test-path> -maxdepth 4 -print 2>/dev/null
# 19. Check whether the cited paths or bytecode artifacts are tracked
git ls-files -- '<src-path>/**' '<test-path>/**' '*__pycache__*' '*.pyc'
# 20. Confirm bytecode ignore policy before deciding whether this is ignored noise
rg -n '__pycache__|\\.py[co]|\\.pyc' .gitignore .git/info/exclude 2>/dev/null
# 21. Read current policy before restoring removed tooling
rg -n 'removed|retired|must not restore|do not restore|changelog|tooling' docs/ README.md CLAUDE.md 2>/dev/null
# 22. Inspect existing guard/helper behavior for ignored-path filtering
rg -n 'ignore|__pycache__|pyc|gitignore|ls-files|rglob|mirror' scripts/ tests/ <src-root>/ 2>/dev/null
# 23. Verification commands after implementing the guard
python scripts/check_unit_test_structure.py --json
python -m pytest tests/unit/validation/test_unit_test_structure.py -q
pre-commit run check-unit-test-structure --files <changed-files>
```
...
| Restoring deleted tooling to satisfy stale path citations (v1.5.0, UNVERIFIED) | Treat the audit's cited source/test directories as proof that the removed package should be recreated | The plan context says the live tree is already free of those paths and repo governance docs say the removed tooling must not be restored | Re-check current paths and policy first; stale citations usually call for a guard/cleanup, not resurrecting deleted packages |
| Trusting plan-time absence assumptions (v1.5.0, UNVERIFIED) | Assume the cited source/test dirs, tracked bytecode, and `__pycache__` artifacts are absent because the plan says they are | The plan is not evidence; a new commit or ignored local artifact can change the tree before implementation | Run `find`, `git ls-files`, and ignore-rule checks at implementation time before editing |
| Relying on a mirror-set helper to catch bytecode-only test skeletons (v1.5.0, UNVERIFIED) | Use the existing source/test mirror computation as the only phantom detector | The helper intentionally ignores no-source/ignored dirs, so `tests/unit/<name>/__pycache__/...pyc` can disappear from the computed mirror set | Add a direct immediate-test-dir phantom detector for dirs containing only ignored noise |
| Treating unverified integration points as facts (v1.5.0, UNVERIFIED) | Assume cited line numbers, docs, hook registrations, console scripts, and helper behavior are correct without opening them in the same turn | Reviewers can NOGO stale coordinates or a guard wired to the wrong hook/entry point even if the concept is sound | Mark every unverified external file/API/hook dependency as an assumption and make it first on the reviewer checklist |
...
- **ProjectHephaestus #1468 (stale audit ghost scaffolding; direct test-dir phantom guard, v1.5.0 case — UNVERIFIED, plan only):**
  - Objective: remediate a stale audit issue by treating cited source/test directories as ghost scaffolding from removed tooling, not by restoring the removed package. **The implementation workflow was not executed end-to-end; this learning is unverified.**
  - Required pre-edit rechecks: `find` on every cited source/test path; `git ls-files` for named paths plus `*.pyc` and `__pycache__`; `.gitignore`/exclude rules for bytecode; current governance docs saying removed changelog/tooling must not be restored; existing test-structure guard and pre-commit hook registrations; helper behavior around ignored/no-source directories.
  - Proposed fix shape: preserve the existing source-side phantom guard; add a dedicated direct test-directory phantom detector for immediate `tests/unit/<name>/` dirs containing only ignored noise such as `__pycache__`; wire a reporter into the orchestrating check; document the module; add focused tests including a pycache-only `tests/unit/<name>` regression and a real-repo absence assertion.
  - Reviewer focus: stale audit citations; plan-time file-existence assumptions; line numbers and docs not verified in the same turn; hook/console-script wiring; helper behavior that filters ignored paths; and whether the regression catches bytecode-only skeletons instead of only normal missing-source mirrors.
  - Verification commands proposed by the plan: absence checks for issue-named dirs; `git ls-files` checks for tracked bytecode; targeted validation pytest; executable structure guard with `--json`; and `pre-commit run check-unit-test-structure --files <changed-files>`.
